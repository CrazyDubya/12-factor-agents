...
class CulturalAnalysis:
    ...
    commentary_sophistication: float  # 0-10 - Nuance and complexity of social commentary
...
class CulturalCommentaryAnalyzer:
    ...
    def __init__(self):
        ...
        self.social_themes = {
            "identity_belonging": {
                "keywords": ["identity", "belonging", "self", "who am i", "place", "home", "roots", "heritage", 
                           "culture", "tradition", "community", "acceptance", "otherness", "outsider"],
                "weight": 1.2,
                "description": "Questions of personal and cultural identity"
            },
            "social_justice": {
                "keywords": ["justice", "equality", "fairness", "rights", "discrimination", "prejudice", 
                           "oppression", "freedom", "liberation", "activism", "protest", "resistance"],
                "weight": 1.5,
                "description": "Issues of fairness and social equity"
            },
            "technology_society": {
                "keywords": ["technology", "digital", "artificial intelligence", "automation", "surveillance", 
                           "privacy", "connection", "isolation", "virtual", "real", "human vs machine"],
                "weight": 1.3,
                "description": "Impact of technology on human society"
            },
            "power_corruption": {
                "keywords": ["power", "authority", "corruption", "abuse", "control", "manipulation", 
                           "government", "politics", "hierarchy", "elite", "system", "establishment"],
                "weight": 1.4,
                "description": "Dynamics of power and its corrupting influence"
            },
            "economic_inequality": {
                "keywords": ["wealth", "poverty", "class", "rich", "poor", "money", "capitalism", "work", 
                           "labor", "exploitation", "privilege", "disadvantage", "opportunity"],
                "weight": 1.3,
                "description": "Economic disparities and class issues"
            },
            "environmental_crisis": {
                "keywords": ["environment", "climate", "nature", "pollution", "destruction", "sustainability", 
                           "future generations", "earth", "planet", "conservation", "crisis"],
                "weight": 1.4,
                "description": "Environmental concerns and sustainability"
            },
            "human_relationships": {
                "keywords": ["love", "family", "friendship", "relationships", "community", "connection", 
                           "loneliness", "isolation", "trust", "betrayal", "communication", "understanding"],
                "weight": 1.1,
                "description": "Human connection and relationship dynamics"
            },
            "generational_conflict": {
                "keywords": ["generation", "old", "young", "tradition", "change", "progress", "past", 
                           "future", "wisdom", "youth", "experience", "innovation"],
                "weight": 1.2,
                "description": "Tensions between different generations"
            },
            "mental_health": {
                "keywords": ["mental health", "depression", "anxiety", "trauma", "healing", "therapy", 
                           "stigma", "wellness", "mind", "psychological", "emotional wellbeing"],
                "weight": 1.3,
                "description": "Mental health awareness and stigma"
            },
            "diversity_inclusion": {
                "keywords": ["diversity", "inclusion", "representation", "minority", "majority", "tolerance", 
                           "acceptance", "difference", "multiculturalism", "racism", "sexism", "bias"],
                "weight": 1.4,
    ...
    def extract_social_content(self, scene: SceneData) -> Dict[str, List[str]]:
        ...
        for theme_category, data in self.social_themes.items():
            ...
            if matches > 0:
                social_content["themes"].append(f"{data['description']}: {matches} references")
        
        # Extract cultural markers
        for marker_category, patterns in self.cultural_markers.items():
            for pattern in patterns:
                if re.search(pattern, all_text):
                    social_content["contemporary_markers"].append(f"{marker_category}: {pattern}")
        ...
        for technique, patterns in self.commentary_techniques.items():
            for pattern in patterns:
                ...
                if matches > 0:
                    social_content["techniques"].append(f"{technique}: {matches} instances")
    ...
    def analyze_social_theme_depth(self, scene: SceneData) -> Tuple[float, List[str]]:
        ...
        for theme_category, data in self.social_themes.items():
            ...
            if theme_count > 0:
                ...
                if theme_count >= 3:
                    ...
                    theme_elements.append(f"{data['description']}: Multi-faceted exploration ({theme_count} aspects)")
                elif theme_count >= 2:
                    theme_complexity += 1.0
                    theme_elements.append(f"{data['description']}: Dual perspective ({theme_count} aspects)")
                else:
                ...
                nuance_count = sum(len(re.findall(pattern, all_text)) for pattern in nuance_patterns)
                if nuance_count > 0:
        ...
        if theme_scores:
            ...
            if theme_variety >= 3:
                depth_score += 2.5
                theme_elements.append(f"Thematic complexity: {theme_variety} social themes")
            elif theme_variety >= 2:
                depth_score += 1.5
                theme_elements.append(f"Dual thematic focus: {theme_variety} social themes")
            ...
            interconnection_count = sum(len(re.findall(pattern, all_text)) for pattern in interconnection_patterns)
            if interconnection_count > 0:
                depth_score += min(2.0, interconnection_count * 0.4)
                theme_elements.append(f"Thematic interconnection: {interconnection_count} connections")
    ...
    def analyze_cultural_relevance(self, scene: SceneData) -> Tuple[float, List[str]]:
        ...
        for marker_category, patterns in self.cultural_markers.items():
            ...
            if category_matches > 0:
                ...
                relevance_indicators.append(f"{marker_category.replace('_', ' ').title()}: {category_matches} references")
        ...
        zeitgeist_count = sum(len(re.findall(pattern, all_text)) for pattern in zeitgeist_patterns)
        if zeitgeist_count > 0:
            relevance_score += min(2.0, zeitgeist_count * 0.5)
            relevance_indicators.append(f"Contemporary zeitgeist: {zeitgeist_count} references")
        
        # Universal themes with contemporary applications
        universal_contemporary = [
            (r'\b(?:communication|misunderstanding|connection)\b', "Digital age communication"),
            (r'\b(?:privacy|surveillance|data|information)\b', "Digital privacy concerns"),
        ...
        bridge_count = sum(len(re.findall(pattern, all_text)) for pattern in generational_bridge)
        if bridge_count > 2:  # Only count if significant
            relevance_score += min(1.5, (bridge_count - 2) * 0.3)
            relevance_indicators.append(f"Cross-generational relevance: {bridge_count} bridges")
        
        final_score = min(10.0, relevance_score)
        
        return final_score, relevance_indicators[:5]
    
    def analyze_commentary_sophistication(self, scene: SceneData) -> Tuple[float, List[str]]:
        ...
        for technique, patterns in self.commentary_techniques.items():
            ...
            if technique_count > 0:
                ...
                sophistication_elements.append(f"{technique.replace('_', ' ').title()}: {technique_count} instances")
        ...
        if technique_scores:
            ...
            if technique_variety >= 3:
                ...
                sophistication_elements.append(f"Multi-technique approach: {technique_variety} styles")
            elif technique_variety >= 2:
                sophistication_score += 2.0
                sophistication_elements.append(f"Dual-technique approach: {technique_variety} styles")
            ...
            weighted_score = sum(count * sophistication_weights.get(technique, 1.0) 
        ...
        if nuance_count > 0:
            ...
            sophistication_elements.append(f"Nuanced perspective: {nuance_count} complexity indicators")
        ...
        intellectual_count = sum(len(re.findall(pattern, all_text)) for pattern in intellectual_patterns)
        if intellectual_count > 0:
            sophistication_score += min(2.0, intellectual_count * 0.4)
            sophistication_elements.append(f"Intellectual depth: {intellectual_count} analytical elements")
        ...
        context_count = sum(len(re.findall(pattern, all_text)) for pattern in context_patterns)
        if context_count > 2:
            sophistication_score += min(1.5, (context_count - 2) * 0.2)
            sophistication_elements.append(f"Contextual awareness: {context_count} context references")
        
        final_score = min(10.0, sophistication_score)
        
        return final_score, sophistication_elements[:5]
    
    def analyze_societal_impact_potential(self, scene: SceneData) -> Tuple[float, List[str]]:
        ...
        emotional_count = sum(len(re.findall(pattern, all_text)) for pattern in emotional_impact_patterns)
        if emotional_count > 0:
            impact_score += min(3.0, emotional_count * 0.6)
            impact_elements.append(f"Emotional resonance: {emotional_count} impact words")
        ...
        action_count = sum(len(re.findall(pattern, all_text)) for pattern in action_patterns)
        if action_count > 0:
        ...
        provocation_count = sum(len(re.findall(pattern, all_text)) for pattern in provocation_patterns)
        if provocation_count > 0:
            impact_score += min(2.0, provocation_count * 0.3)
            impact_elements.append(f"Thought provocation: {provocation_count} challenges")
        ...
        urgency_count = sum(len(re.findall(pattern, all_text)) for pattern in urgency_patterns)
        if urgency_count > 0:
            impact_score += min(2.0, urgency_count * 0.5)
            impact_elements.append(f"Urgency markers: {urgency_count} time-sensitive elements")
        ...
        systemic_count = sum(len(re.findall(pattern, all_text)) for pattern in systemic_patterns)
        if systemic_count > 0:
            impact_score += min(1.5, systemic_count * 0.4)
            impact_elements.append(f"Systemic change potential: {systemic_count} system references")
    ...
    def identify_primary_social_theme(self, scene: SceneData) -> str:
        ...
        theme_scores = {}
        for theme_category, data in self.social_themes.items():
            score = sum(1 for keyword in data["keywords"] if keyword in all_text) * data["weight"]
            if score > 0:
    ...
    def determine_cultural_perspective(self, scene: SceneData) -> str:
        ...
        # Temporal markers
        if re.search(r'\b(?:future|tomorrow|2030|2040|2050|sci-fi|futuristic)\b', all_text):
            return "Futuristic"
        elif re.search(r'\b(?:history|past|traditional|ancient|classical)\b', all_text):
            return "Historical"
        elif re.search(r'\b(?:today|current|modern|contemporary|now|2020s)\b', all_text):
            return "Contemporary"
        else:
            return "Universal"
    ...
    def determine_commentary_approach(self, scene: SceneData) -> str:
        ...
        if approach_scores:
            ...
            return primary_approach.replace('_', ' ').title().replace(' Approach', '').replace(' Commentary', '')
        else:
            return "Subtle"
    ...
    def analyze_scene_cultural_commentary(self, scene: SceneData) -> CulturalAnalysis:
        ...
        sophistication_score, sophistication_elements = self.analyze_commentary_sophistication(scene)
        ...
        overall_score = (theme_score + relevance_score + sophistication_score + impact_score) / 4
        ...
        print(f"   🎯 Focus: {primary_theme[:30]}... | Perspective: {cultural_perspective} | Approach: {commentary_approach}")
...
def run_cultural_commentary_analysis():
        ...
        try:
            ...
            result = {
                ...
                "cultural_scores": {
                    ...
                    "cultural_relevance_score": round(analysis.cultural_relevance_score, 2),
                    "commentary_sophistication": round(analysis.commentary_sophistication, 2),
                    "societal_impact_potential": round(analysis.societal_impact_potential, 2)
                },
    ...
    results.sort(key=lambda x: x['cultural_scores']['overall_cultural_score'], reverse=True)
    ...
    if results:
        ...
        cultural_scores = [r['cultural_scores']['overall_cultural_score'] for r in results]
        ...
        # Component averages
        theme_avg = sum(r['cultural_scores']['social_theme_depth'] for r in results) / len(results)
        relevance_avg = sum(r['cultural_scores']['cultural_relevance_score'] for r in results) / len(results)
        sophistication_avg = sum(r['cultural_scores']['commentary_sophistication'] for r in results) / len(results)
        impact_avg = sum(r['cultural_scores']['societal_impact_potential'] for r in results) / len(results)
        ...
        for result in results:
            ...
            print(f"   Overall: {scores['overall_cultural_score']}/10 | Genre: {result['genre']}")
            print(f"   Theme:{scores['social_theme_depth']:.1f} Relevance:{scores['cultural_relevance_score']:.1f} Sophist:{scores['commentary_sophistication']:.1f} Impact:{scores['societal_impact_potential']:.1f}")
            print(f"   Focus: {chars['primary_social_theme'][:40]}...")
            print(f"   Perspective: {chars['cultural_perspective']} | Approach: {chars['commentary_approach']} | Scope: {chars['societal_scope']}")
            print()
        ...
        print("🌍🖖 Cultural & Social Commentary Evaluation complete - KEPT ON TREKKIN!")
