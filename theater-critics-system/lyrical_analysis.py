...
class LyricalContentAnalyzer:
    ...
    def analyze_rhyme_scheme(self, lyrics: List[str]) -> Tuple[float, List[str]]:
        ...
        sophisticated_patterns = sum(1 for p in rhyme_patterns if "ABAB" in p or "ABCB" in p)
    ...
    def analyze_metaphors(self, lyrics: List[str]) -> Tuple[float, List[str]]:
        ...
        metaphor_indicators = [
            r'\bis\s+(?:like\s+)?a\s+', r'\bare\s+(?:like\s+)?',
            r'\blike\s+', r'\bas\s+', r'\bthan\s+',
            r'\bheart\s+of\s+', r'\bsoul\s+of\s+',
            r'\bdance\s+of\s+', r'\bsong\s+of\s+',
            r'\bfire\s+', r'\bflame\s+', r'\blight\s+', r'\bdarkness\s+',
            r'\bstorm\s+', r'\briver\s+', r'\bocean\s+', r'\bmountain\s+'
        ]
    ...
    def analyze_thematic_coherence(self, lyrics: List[str], scene_title: str) -> Tuple[float, List[str]]:
    ...
    async def get_ai_lyrical_analysis(self, scene: SceneData, lyrics: List[str]) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'improvements': improvements if improvements else ["Analysis refinement needed"], 
                'commentary': commentary if commentary else "Specialized lyrical analysis completed"
            }
...
async def test_lyrical_analysis():
    ...
    if results:
        ...
        for i, result in enumerate(results, 1):
            ...
            print(f"   Rhyme: {result.rhyme_scheme_score:.1f} | Metaphor: {result.metaphor_usage_score:.1f}")
            print(f"   Theme: {result.thematic_coherence_score:.1f} | Vocab: {result.lyrical_sophistication_score:.1f} ({result.vocabulary_complexity})")
